...
var editorConfig = {
  ...
  ckfinder: {
    ...
    }/editable-area/upload-image?token=${getCookie("token")}`,
  },
  heading: {
    options: [
...
const EditableArea = ({
  ...
  useEffect(() => {
    if (editableAreaModels) {
      const editableArea = editableAreaModels.find(
        (q) => q.guid == guid && q.pathname == pathname
      );
      if (editableArea) {
        if (editableArea.message) {
          setValues({
            pageError: "No page with that URL was found",
          });
        } else {
          setValues({
            ...values,
  ...
  useEffect(() => {
    if (publishEditableAreas) {
      updateEditableAreas({ guid, pathname, data, link, isEditablePage });
    }
  }, [publishEditableAreas]);

  useEffect(() => {
    updateEditableAreas({ guid, pathname, link, isEditablePage });
  }, []);
